...
def main():
    ...
    if args.experiment_name is not None and rank == 0:
        ...
        experiment.add_tags(["ddp", "llm-training", "pytorch", "distributed"])
        ...
        experiment.log_parameters(
            {
                ...
                "optimizer": "ZeroRedundancyOptimizer(AdamW)",
    ...
    ddp_mem_stats = get_mem_stats(device)
    LOGGER.info(f"After DDP: model uses {ddp_mem_stats['curr_alloc_gb']}gb")

    # Log DDP memory stats to Comet
    if experiment:
        experiment.log_metrics(
            {
                "initial_memory_gb": ddp_mem_stats["curr_alloc_gb"],
            },
            step=0,
        )
...
def _get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("-e", "--experiment-name", default="ddp")
